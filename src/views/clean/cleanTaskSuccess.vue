...
<div>
  ...
  <div class="success_content">
    ...
    <div style="margin-left:5%;">
      ...
      <el-button type="primary" @click="nextStep" style="position:relative; bottom:-185px;left:35%">下一步</el-button>
</div>
</template>
<script>
import CleanNav from '@/components/clean_nav';
import { getConfigSuccess } from '@/api/api';
...
export default {
    ...
    methods:{
        initData:function(){
          ...
          this.loadConfigSuccessData();
        },
        loadConfigSuccessData(){
             getConfigSuccess({params:{id: this.$route.query.id}}).then(data => {
                  ...
                  if (code !== 200) {
                    ...
                    this.database = result.success_writer.database;
                    this.table = result.success_writer.table;
                    this.tableData = result.success_writer.fields
      ...
      backStep(){
          var url_path = "/clean/task/tranform?id=" + this.$route.query.id;
          this.$router.push(url_path);
      }
    }
}
